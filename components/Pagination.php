...
class Pagination
{
    ...
    public function get()
    {
        if ($this->total > $this->limit)
        {
            ...
            if (!is_null($links)) {
                ...
                {
                    $links = $this->generateHtml($this->current_page - 1, '&lt;', 'Предыдущая страница') . $links;
                    # Создаём ссылку "На первую"
                    $links = $this->generateHtml(1, '&laquo;', 'Первая страница') . $links;
                ...
                {
                    if ($this->amount != $this->current_page)
                    {
                        $links .= $this->generateHtml($this->current_page + 1, '&gt;', 'Следующая страница');

                    }
                    # Создаём ссылку "На последнюю"
                    $links .= $this->generateHtml($this->amount, '&raquo;', 'Последняя страница');
                }
            }
    ...
    private function generateHtml($page, $text = null, $title = null)
    {
        ...
        return
                '<a href="' . $currentURI . $this->index . $page . '"><li'. $title .'>' . $text . '</li></a>';
    }
